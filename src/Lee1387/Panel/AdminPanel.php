...
class AdminPanel
{
    ...
    {
        $onlinePlayers = AntiCheat::getInstance()->getServer()->getOnlinePlayers();
    ...
    private static function createStats(Player $player, Player $statsPlayer): void 
    {
        ...
        $statsUser = AntiCheat::getInstance()->getUserManager()->getUser($statsPlayer->getUniqueId()->toString());
    ...
    public static function openOtherSettings(Player $player): void 
    {
            ...
            for ($i = 0; $i < count($data); $i++){
                switch ($i){
                    case 0:
                        ...
                        break;
                    case 1:
                        AntiCheat::getInstance()->getSavedConfig()->set("enable-debug", $data[$keys[$i]]);
                        break;
                    case 2:
                        AntiCheat::getInstance()->getSavedConfig()->set("prefix", $data[$keys[$i]]);
                        break;
                    case 3:
                        AntiCheat::getInstance()->getSavedConfig()->set("alert-message-debug", $data[$keys[$i]]);
                        break;
                    case 4:
                        AntiCheat::getInstance()->getSavedConfig()->set("alert-message", $data[$keys[$i]]);
                        break;
                    case 5:
                        AntiCheat::getInstance()->getSavedConfig()->set("kick-message", $data[$keys[$i]]);
                        break;
                    case 6:
                        AntiCheat::getInstance()->getSavedConfig()->set("ban-message", $data[$keys[$i]]);
                        break;
                    default:
                        break;
                }
        ...
        $form->addToggle("Debug Mode (Global Setting)", AntiCheat::getInstance()->debugEnabled());
        $form->addInput("Prefix", "", AntiCheat::getInstance()->getSavedConfig()->get("prefix"));
        $form->addInput("Alert-Message-Debug", "", AntiCheat::getInstance()->getSavedConfig()->get("alert-message-debug"));
        $form->addInput("Alert-Message", "", AntiCheat::getInstance()->getSavedConfig()->get("alert-message"));
        $form->addInput("Kick-Message", "", AntiCheat::getInstance()->getSavedConfig()->get("kick-message"));
        $form->addInput("Ban-Message", "", AntiCheat::getInstance()->getSavedConfig()->get("ban-message"));
        $player->sendForm($form);
    ...
    {
        $checks = AntiCheat::getInstance()->getCheckManager()->getChecks();
            ...
            for ($i = 0; $i < count($data) - 1; $i++){
                ...
                if (isset($checks[$keys[$i]])) {
                    ...
                    if ($check instanceof Check) {
                        ...
                        AntiCheat::getInstance()->getSavedConfig()->set($check->getName() . "-notify", $data[$keys2[$i]] ?? false);
                    }
                }
            }
            ...
            for ($i = 0; $i < count($data); $i++){
                ...
                if (isset($checks[$keys[$i]])) {
                    ...
                    if ($check instanceof Check) {
                        ...
                        AntiCheat::getInstance()->getSavedConfig()->set($check->getName() . "-enabled", $data[$keys2[$i]] ?? false);
                    }
                }
            }
            ...
            for ($i = 0; $i < count($data); $i++){
                ...
                if (isset($checks[$keys[$i]])) {
                    ...
                    if ($check instanceof Check) {
                        ...
                        AntiCheat::getInstance()->getSavedConfig()->set($check->getName() . "-AlertFrequency", $data[$keys2[$i]] ?? 25);
                    }
                }
            }
            ...
            for ($i = 0; $i < count($data); $i++){
                ...
                if (isset($checks[$keys[$i]])) {
                    ...
                    if ($check instanceof Check) {
                        ...
                        AntiCheat::getInstance()->getSavedConfig()->set($check->getName() . "-MaxViolations", $data[$keys2[$i]] ?? 25);
                    }
                }
            }
    ...
    {
        $punishments = AntiCheat::getInstance()->getCheckManager()->getPunishments();
        $checks = AntiCheat::getInstance()->getCheckManager()->getChecks();
        $newChecks = [];
        $keys = array_keys($checks);
            ...
            for ($i = 0; $i < count($data); $i++){
                ...
                if (isset($newChecks[$keys[$i]])) {
                    ...
                    if ($check instanceof Check) {
                        ...
                        AntiCheat::getInstance()->getSavedConfig()->set($check->getName() . "-Punishment", $punishments[$data[$keys2[$i]]] ?? "Cancel");
                    }
                }
            }
        ...
        foreach($checks as $check) {
            if ($check->getCategory() == CheckManager::MOVEMENT || $check->getCategory() == CheckManager::PLAYER) {
                $form->addDropdown($check->getName(), $punishments, AntiCheat::getInstance()->getCheckManager()->getPunishmentID($check->getPunishment()));
            }
        }
